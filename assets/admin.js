...
function bind(){
  ...
  on('#fiscalYear','change',()=>{dataset.fiscalYear=+$('#fiscalYear').value;const y=$('#sumYear');if(y)y.textContent=dataset.fiscalYear;loadPreparedRca();if(dataset.rows.length)validateAndRender();if($('#rcaCenter')?.classList.contains('active'))loadRcaIncidents()});
  ...
  on('#downloadRcaManifest','click',downloadCurrentRcaManifest);
  on('#clearPreparedRca','click',clearPreparedRca);
...
function rcaDraftKey(){return `drms_rca_prepared_${$('#fiscalYear')?.value||2569}`}
function loadPreparedRca(){
  try{rcaState.prepared=JSON.parse(localStorage.getItem(rcaDraftKey())||'[]')}catch{rcaState.prepared=[]}
  renderPreparedRca()
}
function savePreparedRca(){
  localStorage.setItem(rcaDraftKey(),JSON.stringify(rcaState.prepared));
  renderPreparedRca()
}
function clearPreparedRca(){
  rcaState.prepared=[];
  localStorage.removeItem(rcaDraftKey());
  renderPreparedRca();
  toast('ล้างรายการที่เตรียมไว้แล้ว')
}
...
function incidentId(r,index){
  ...
  return id?id.replace(/[^\w\-]/g,'_'):`INC_${$('#fiscalYear')?.value||2569}_${String(index+1).padStart(5,'0')}`
}
async function loadRcaIncidents(){
  ...
  try{
    ...
    renderRcaRows();
    toast(`พบรายการระดับ E–I และ 3–5 จำนวน ${rcaState.rows.length.toLocaleString()} รายการ`)
...
async function loadRcaManifest(showToast=true){
  try{
    ...
    try{
      const extraRes=await fetch(`config/rca-2568-additions.json?v=${Date.now()}`);
      if(extraRes.ok){
        const extraObj=await extraRes.json();
        const extraItems=Array.isArray(extraObj)?extraObj:(extraObj.items||[]);
        for(const item of extraItems){
          rcaState.manifest=rcaState.manifest.filter(x=>!(String(x.incident)===String(item.incident)&&Number(x.year)===Number(item.year)));
          rcaState.manifest.push(item)
        }
      }
    }catch(e){console.warn('โหลด RCA ปี 2568 ชุดเพิ่มเติมไม่สำเร็จ',e)}
    renderRcaRows();
    if(showToast)toast('โหลด rca.json ล่าสุดแล้ว')
...
function preparedEntry(id,year){
  return rcaState.prepared.find(x=>String(x.incident)===String(id)&&Number(x.year)===Number(year))
}
...
function renderRcaRows(){
  ...
  $('#rcaRows').innerHTML=rows.map(x=>{
    const r=x.raw,e=rcaEntry(x.id,year),p=preparedEntry(x.id,year);
    return `<tr>
      <td>${esc(fmtDate(r[IDX.date]))}</td>
      <td><b>${esc(x.id)}</b></td>
      <td>${esc(r[IDX.risk])}</td>
      <td>${esc(r[IDX.mainUnit])}</td>
      <td><span class="severity-pill">${esc(r[IDX.sev])}</span></td>
      <td>${e?'<span class="rca-has">● มี RCA</span>':'<span class="rca-missing">● ไม่มี RCA</span>'}${p&&!e?'<small class="prepared-note">เตรียมไฟล์แล้ว</small>':''}</td>
      <td>${e?`<a class="download-link" href="${esc(e.file)}" target="_blank" rel="noopener">ดาวน์โหลด RCA</a>`:'–'}</td>
      <td><button class="prepare-rca" data-incident="${esc(x.id)}">${e?'เลือกไฟล์แทนที่':'เลือกไฟล์ RCA'}</button></td>
    </tr>`
  }).join('')||'<tr><td colspan="8" class="quality-empty">ไม่พบรายการตามตัวกรอง</td></tr>';
...
function mergedRcaManifest(){
  const year=+$('#fiscalYear').value;
  let manifest=Array.isArray(rcaState.manifest)?[...rcaState.manifest]:[];
  for(const item of rcaState.prepared){
    manifest=manifest.filter(x=>!(String(x.incident)===String(item.incident)&&Number(x.year)===Number(item.year)));
    manifest.push({
  ...
  return manifest.sort((a,b)=>Number(a.year)-Number(b.year)||String(a.incident).localeCompare(String(b.incident)))
}
function downloadCurrentRcaManifest(){
  const manifest=mergedRcaManifest();
  download('\ufeff'+JSON.stringify(manifest,null,2),'rca.json','application/json');
  addLog('Download RCA manifest',manifest.length,'rca.json');
  toast(`ดาวน์โหลด rca.json จำนวน ${manifest.length.toLocaleString()} รายการแล้ว`)
}
function renderPreparedRca(){
  const el=$('#preparedRcaList');if(!el)return;
  const year=+($('#fiscalYear')?.value||2569);
  const items=rcaState.prepared.filter(x=>Number(x.year)===year);
  el.innerHTML=items.length?items.map(x=>`<div class="prepared-rca-item">
    <div><b>${esc(x.filename)}</b><small>Incident ${esc(x.incident)} • ปี ${esc(x.year)}</small></div>
    <code>drms-v6/rca/${esc(x.year)}/${esc(x.filename)}</code>
    <button class="remove-prepared" data-incident="${esc(x.incident)}">นำออก</button>
  </div>`).join(''):'<div class="quality-empty">ยังไม่มีไฟล์ที่เตรียมไว้</div>';
  $$('.remove-prepared').forEach(b=>b.onclick=()=>{
    rcaState.prepared=rcaState.prepared.filter(x=>!(String(x.incident)===String(b.dataset.incident)&&Number(x.year)===year));
    savePreparedRca();renderRcaRows()
  })
}
async function uploadSelectedRca(file){
  ...
  rcaState.prepared=rcaState.prepared.filter(x=>!(String(x.incident)===String(incident)&&Number(x.year)===year));
  rcaState.prepared.push({
    incident,
    year,
    file:publicPath,
    filename:renamed,
  ...
  savePreparedRca();
  renderRcaRows();
  addLog('Prepare RCA file',1,renamed);
  toast(`ดาวน์โหลด ${renamed} แล้ว กรุณากดดาวน์โหลด rca.json เมื่อเตรียมครบ`)
}
...
applySettings();loadPreparedRca();bind();renderAll();